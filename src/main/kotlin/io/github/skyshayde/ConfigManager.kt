...
class ConfigManager {
    ...
    private val guilds: ObjectRepository<GuildData> = db.getRepository(GuildData::class.java)



    fun getRoles(guild : IGuild) : Map<String, String> {
        val guild : GuildData? = guilds.find(ObjectFilters.eq("ID", guild.longID)).firstOrNull()
        if(guild != null) {
            return guild.roles
        }
        return emptyMap()
